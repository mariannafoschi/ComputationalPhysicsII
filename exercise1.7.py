...
h_E = 3.5/5.9*epsilon/(N_E)  /6                   # energy step (to get just a bit below E_max = 5.9 meV)
E_range = (np.array(range(N_E))+1)*h_E+0.5          # array with energy values
E_peak = [0.0745,0.233,0.4456]                               # initialize a vector where we write the cross sections
h = 0.001                                       # step
r_low = sigma*0.5
r_max = sigma*5.6             # lowest mesh point
...
phase_peak = np.zeros([3,ell_max+1])                   # initialize a vector where we write the phase shifts
...
for j in range(3):                                      
    ...
    # Calculate phase shifts
    for ell in range(ell_max+1):                # cycle over the values of angular momenta
        y = numerov_LJ(E_peak[j],ell,x,h)      # calculate w.f. for a specific ell
        kappa = (y[r1_ix]*x[r2_ix])/(y[r2_ix]*x[r1_ix])
        phase_peak[j,ell] = np.arctan((kappa*bess(ell,(k*x[r2_ix]))-bess(ell,(k*x[r1_ix])))/(kappa*neumm(ell,(k*x[r2_ix]))-neumm(ell,(k*x[r1_ix]))))
        crossect_partial_peak[j,ell] = (4*pi/(prefac*E_peak[j]))*(2*ell+1)*np.sin(phase_peak[j,ell])**2
        
    plt.plot(range(ell_max+1),crossect_partial_peak[j,:])
plt.show()
